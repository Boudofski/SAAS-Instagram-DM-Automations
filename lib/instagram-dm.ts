...
type CtaMode = "button_template" | "follow_gate_card" | "text_link_fallback" | "none";
...
export async function sendInstagramDirectResponse(params: {
  ...
  try {
    ...
    try {
      ...
      if (params.followGatePrompt && shouldTryTextFallback(error)) {
...
export async function sendInstagramCommentPrivateReply(params: {
  ...
  const followGatePayload = params.followGatePrompt
    ...
    : null;
  const preferred = followGatePayload
  ...
  const textFallback = followGatePayload
  ...
  const usesTemplate = preferred.ctaMode === "button_template" || preferred.ctaMode === "follow_gate_card";

"use client";
import { LocalizedInput, LocalizedButton, LocalizedTextarea } from "@/components/i18n/localized-controls";
    ...
    <div className="mt-3 flex h-[calc(100dvh-9.5rem)] min-h-[420px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-[#f5f6fa] text-slate-950 shadow-sm dark:border-white/10 dark:bg-[#050816] dark:text-white sm:h-[calc(100dvh-7.5rem)] sm:min-h-[520px]">
      <header className={["shrink-0 flex-wrap items-center gap-2 border-b border-slate-200 bg-white/95 px-3 py-2 backdrop-blur dark:border-white/10 dark:bg-[#080c18]/95 sm:px-4 sm:py-3 lg:flex-nowrap lg:gap-3", selectedId ? "hidden md:flex" : "flex"].join(" ")}>
        ...
        <label className="relative order-3 w-full lg:order-none lg:mx-auto lg:max-w-xl">
          ...
          <LocalizedInput value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search conversations" className="ap3k-input w-full rounded-xl py-2.5 ps-11 pe-4 text-sm sm:py-3" />
        </label>
        <div className="ms-auto flex shrink-0 items-center gap-2">
          ...
          <LocalizedButton type="button" onClick={() => void refreshAll(true)} aria-label="Refresh conversations and messages" className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 dark:border-white/10 dark:bg-white/[0.05] dark:text-slate-300 dark:hover:bg-white/10 sm:h-11 sm:w-11"><RefreshCw className={refreshing ? "h-4 w-4 animate-spin" : "h-4 w-4"} /></LocalizedButton>
        </div>
      </header>
      ...
      <div className={["grid min-h-0 flex-1 overflow-hidden bg-white dark:bg-[#0d1220] md:grid-cols-[300px_minmax(0,1fr)]", foldersExpanded ? "xl:grid-cols-[176px_320px_minmax(0,1fr)]" : "xl:grid-cols-[64px_320px_minmax(0,1fr)]"].join(" ")}>
        <nav aria-label="Inbox folders" className="hidden min-h-0 border-e border-slate-200 bg-slate-50/70 p-2 dark:border-white/10 dark:bg-white/[0.02] xl:flex xl:flex-col">
          ...
          <LocalizedButton type="button" title="Unread" aria-label="Unread" onClick={() => setFilter("unread")} className={folderClass(filter === "unread", foldersExpanded)}><Instagram className="h-4 w-4 shrink-0" />{foldersExpanded ? <><span className="flex-1 text-start"><UiText>{"Unread"}</UiText></span><span>{unreadTotal}</span></> : null}</LocalizedButton>
          <LocalizedButton type="button" title="Recent" aria-label="Recent" onClick={() => setFilter("recent")} className={folderClass(filter === "recent", foldersExpanded)}><Clock3 className="h-4 w-4 shrink-0" />{foldersExpanded ? <span className="flex-1 text-start"><UiText>{"Recent"}</UiText></span> : null}</LocalizedButton>
        </nav>

        <aside className={["min-h-0 flex-col overflow-hidden border-e border-slate-200 dark:border-white/10", selectedId ? "hidden md:flex" : "flex"].join(" ")}>
          <div className="flex h-14 items-center gap-2 border-b border-slate-200 px-3 dark:border-white/10">
            <label className="relative">
              <span className="sr-only"><UiText>{"Conversation folder"}</UiText></span>
              <select value={filter} onChange={(event) => setFilter(event.target.value as ConversationFilter)} className="appearance-none rounded-lg border border-slate-200 bg-transparent py-2 ps-3 pe-8 text-xs font-bold text-slate-600 outline-none dark:border-white/10 dark:text-slate-300">
              ...
              <ChevronDown className="pointer-events-none absolute end-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2" />
            </label>
            <button type="button" onClick={() => setSort((value) => value === "newest" ? "oldest" : "newest")} className="ms-auto inline-flex items-center gap-1 rounded-lg px-2 py-2 text-[11px] font-bold text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-white"><ArrowDownUp className="h-3.5 w-3.5" /> <UiText>{sort === "newest" ? "Newest" : "Oldest"}</UiText></button>
          </div>
          ...
          {!selected ? <EmptyInbox /> : (
            <>
              ...
              <div ref={messagesViewportRef} className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain bg-[radial-gradient(circle_at_top,#faf5ff_0,transparent_42%)] px-4 py-6 dark:bg-[radial-gradient(circle_at_top,rgba(109,40,217,0.12)_0,transparent_42%)] sm:px-7">
                <div className="mx-auto mb-7 flex max-w-sm flex-col items-center text-center"><Avatar src={selected.profilePictureUrl} name={selected.recipientUsername || selected.recipientIgId} size="xl" /><p className="mt-3 text-sm font-black">{displayName(selected)}</p><p className="text-xs text-slate-400"><UiText>{"Instagram conversation"}</UiText></p>{selected.automation?.name ? <span className="mt-2 rounded-full bg-slate-100 px-3 py-1 text-[10px] font-bold text-slate-500 dark:bg-white/10 dark:text-slate-300"><UiText>{"Started by "}</UiText>{selected.automation.name}</span> : null}</div>
              ...
              <footer className="border-t border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-[#0d1220] sm:p-4">
                {error && <p className="mb-2 rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-600 dark:bg-red-500/10 dark:text-red-300"><UiText>{error}</UiText></p>}
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-2 transition focus-within:border-ef-purple/40 focus-within:ring-2 focus-within:ring-rf-purple/10 dark:border-white/10 dark:bg-white/[0.035]">
                  <LocalizedTextarea value={draft} maxLength={1000} rows={2} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder={isReplyWindowOpen(selected.lastInboundAt) ? "Reply to this Instagram conversation…" : "Instagram's 24-hour reply window is closed"} disabled={!isReplyWindowOpen(selected.lastInboundAt)} className="w-full resize-none bg-transparent px-2 py-2 text-sm outline-none placeholder:text-slate-400 disabled:cursor-not-allowed disabled:opacity-60" />
                  <div className="relative flex items-center gap-1 border-t border-slate-200 pt-2 dark:border-white/10">
                    <LocalizedButton type="button" onClick={() => setEmojiOpen((value) => !value)} aria-expanded={emojiOpen} aria-label="Add emoji" className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 hover:bg-white hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-white"><Smile className="h-4 w-4" /></LocalizedButton>
                    {emojiOpen && <div className="absolute bottom-10 start-0 z-20 flex flex-wrap gap-1 rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-white/10 dark:bg-[#171d2b]">{["😊","👍","❤️","✨","🔥","🎁","🙏","👏"].map((emoji) => <button key={emoji} type="button" onClick={() => { setDraft((value) => `${value}${emoji}`); setEmojiOpen(false); }} className="grid h-8 w-8 place-items-center rounded-lg text-lg hover:bg-slate-100 dark:hover:bg-white/10">{emoji}</button>)}</div>}
                    <span className="ms-auto hidden text-[10px] text-slate-400 sm:inline"><UiText>{"Enter to send · Shift + Enter for a new line"}</UiText></span><button type="button" onClick={() => void send()} disabled={sending || !draft.trim() || !isReplyWindowOpen(selected.lastInboundAt)} className="ms-2 inline-flex h-9 items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-pink-500 px-4 text-xs font-black text-white shadow-sm disabled:opacity-35">{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}<span className="hidden sm:inline"><UiText>{"Send"}</UiText></span></button>
                  </div>
                </div>
              </footer>
            </>
...
function ConversationRow({ conversation, selected, onClick }: { conversation: any; selected: boolean; onClick: () => void }) {
  ...
  return <button type="button" onClick={onClick} className={["flex w-full items-start gap-3 border-b border-slate-100 p-4 text-start transition dark:border-white/[0.07]", selected ? "bg-violet-50 dark:bg-violet-500/10" : "hover:bg-slate-50 dark:hover:bg-white/[0.04]"].join(" ")}><Avatar src={conversation.profilePictureUrl} name={conversation.recipientUsername || conversation.recipientIgId} size="lg" /><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="truncate text-sm font-black">{displayName(conversation)}</span><span className="ms-auto shrink-0 text-[10px] text-slate-400">{relativeTime(conversation.lastMessageAt, locale)}</span></span><span className={conversation.unreadCount ? "mt-1 block truncate text-xs font-bold text-slate-800 dark:text-white" : "mt-1 block truncate text-xs text-slate-500 dark:text-slate-400"}>{preview}</span><span className="mt-2 flex items-center gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-slate-500 dark:bg-white/[0.07] dark:text-slate-400">{conversation.automation?.source ?? "manual"}</span>{conversation.unreadCount > 0 ? <span className="ms-auto grid h-5 min-w-5 place-items-center rounded-full bg-rf-purple px-1 text-[10px] font-black text-white">{conversation.unreadCount}</span> : <CheckCheck className="ms-auto h-3.5 w-3.5 text-rf-purple/70" />}</span></span></button>;
}
...
function EmptyInbox({ filtered = false }: { filtered?: boolean }) { return <div className="grid min-h-64 place-items-center p-8 text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-rf-purple/10 text-rf-purple"><Inbox className="h-6 w-6" /></span><p className="mt-4 font-black"><UiText>{filtered ? "No matching chats" : "No conversations yet"}</UiText></p><p className="mt-1 max-w-xs text-sm text-slate-500 dark:text-slate-400"><UiText>{filtered ? "Try a different search or show all chats." : "New Instagram DMs and story interactions will appear here automatically."}</UiText></p></div></div>; }
...
function relativeTime(value: string | Date, locale: Locale) {

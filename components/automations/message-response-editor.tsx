        ...
        <div className="space-y-3">
          {buttons.map((button, index) => (
            <div key={index} className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-950/35 sm:grid-cols-[minmax(150px,0.7fr)_minmax(220px,1.3fr)_auto] sm:items-end">
              <label className="min-w-0 text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400"><UiText>{"Button label"}</UiText><input value={button.label} maxLength={20} onChange={(event) => updateButton(index, { label: event.target.value })} placeholder={`Link ${index + 1}`} className="ap3k-input mt-2 w-full rounded-xl px-4 py-3 text-sm normal-case tracking-normal" />
              </label>
              <label className="min-w-0 text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400"><UiText>{"Destination URL"}</UiText><input type="url" inputMode="url" value={button.url} onChange={(event) => updateButton(index, { url: event.target.value })} placeholder="https://your-site.com/offer" className="ap3k-input mt-2 w-full rounded-xl px-4 py-3 text-sm normal-case tracking-normal" />
              </label>
              {buttons.length > 1 ? (

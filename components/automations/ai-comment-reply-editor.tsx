...
export default function AiCommentReplyEditor({ enabled, available, workspaceReady, planLabel, instructions, settingsHref = "#", onChange }: Props) {
  ...
  return (
    <section className={`overflow-hidden rounded-2xl border transition-colors ${enabled ? "border-violet-400/35 bg-violet-500/[0.06]" : "border-slate-200 bg-white dark:border-white/10 dark:bg-white/[0.03]"}`}>
      ...
      {locked ? <div className="border-t border-slate-200 px-5 py-3 text-xs text-slate-500 dark:border-white/10 dark:text-slate-400">{planLabel === "Free" ? <UiMessage source="Your Free plan includes saved replies. {upgrade} for AP3K AI." values={{ upgrade: <Link href={settingsHref.replace(/\/ai$/, "/billing")} className="font-black text-violet-600 dark:text-violet-300"><UiText>{"Upgrade to Pro"}</UiText></Link> }} /> : !workspaceReady ? <UiMessage source="Enable the AI Comments master switch in {settings} before using it here." values={{ settings: <Link href={settingsHref} className="font-black text-violet-600 dark:text-violet-300"><UiText>{"AP3K AI"}</UiText></Link> }} /> : <UiText>{"AP3K AI is unavailable right now."}</UiText>}</div> : null}
      {enabled ? <div className="space-y-4 border-t border-violet-500/15 p-5"><div className="rounded-xl border border-violet-500/15 bg-violet-500/[0.05] p-4"><p className="text-xs font-black uppercase tracking-wider text-violet-600 dark:text-violet-300"><UiText>{"Uses AP3K AI settings"}</UiText></p><p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400"><UiText>{"Tone, knowledge, brand voice, guardrails, and comment protection are managed once in AP3K AI."}</UiText></p><Link href={settingsHref} className="mt-3 inline-flex items-center gap-1.5 text-xs font-black text-violet-600 dark:text-violet-300"><Settings2 className="h-3.5 w-3.5" /> <UiText>{"Open AP3K AI"}</UiText></Link></div><label className="block"><span className="flex items-center justify-between gap-3 text-xs font-black uppercase tracking-wider text-slate-500"><span><UiText>{"Automation focus"}</UiText>{" "}<span className="font-semibold normal-case tracking-normal text-slate-400"><UiText>{"(optional)"}</UiText></span></span><span>{instructions.length}/1600</span></span><textarea value={instructions} onChange={(event) => onChange({ instructions: event.target.value })} maxLength={1600} rows={4} dir="auto" placeholder={tr("Example: Focus on this product launch. Mention the launch date only when asked.")} className="ap3k-textarea mt-2 w-full resize-y rounded-xl px-4 py-3 text-sm" /></label></div> : null}
    </section>
  );
}

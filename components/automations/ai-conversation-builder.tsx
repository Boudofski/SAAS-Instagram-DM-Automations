...
export default function AiConversationBuilder({ slug, integrationId, accountName, automationId, automation }: Props) {
  ...
  return <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-950 dark:border-white/10 dark:bg-[#0c101b] dark:text-slate-50">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-white/10">
      <div className="flex items-center gap-3"><Link href={`/dashboard/${slug}/automation`} aria-label="Back to automations" className={secondary}><ArrowLeft size={16} /></Link><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-violet-600 dark:text-violet-300">Flow Builder</p><h1 className="text-lg font-bold tracking-tight">Automate conversations with AI</h1></div></div>
      <div className="flex items-center gap-2"><button type="button" className={`${secondary} lg:hidden`} onClick={() => setMobilePreview(value => !value)}>{mobilePreview ? "Edit flow" : "Preview"}</button><button type="button" disabled={!valid || blocked} onClick={() => setStage(stage === "configure" ? "flow" : "configure")} className={button}>{stage === "configure" ? "Continue" : "Edit AI"}<ChevronRight size={16} /></button></div>
    </header>
    <div className="grid min-w-0 lg:grid-cols-[minmax(380px,0.95fr)_minmax(360px,1.05fr)]">
      <section aria-label="AI conversation settings" className={`${mobilePreview ? "hidden lg:flex" : "flex"} min-w-0 flex-col lg:h-[calc(100dvh-13rem)] lg:min-h-[620px]`}>
      ...
      <aside aria-label="Conversation preview" className={`${mobilePreview ? "flex" : "hidden lg:flex"} min-w-0 flex-col items-center justify-center border-s border-slate-200 bg-[radial-gradient(ellipse_at_top_left,_#ede9fe,_#faf5ff_45%,_#fff7ed)] p-5 dark:border-white/10 dark:bg-[radial-gradient(ellipse_at_top_left,_#271944,_#111424_55%,_#171020)] sm:p-8`}>
        ...
        <div className="flex h-[640px] max-h-[78dvh] min-h-[450px] w-full max-w-[370px] flex-col overflow-hidden rounded-[44px] border-[10px] border-[#202330] bg-[#101115] text-white shadow-2xl">
          ...
          {mobilePreview && error ? <p role="alert" className="px-4 py-2 text-xs text-red-300 lg:hidden">{error}</p> : null}

...
export default function EditorPreview({ commentPreview, data, mode, onModeChange, username, avatar, source = "COMMENT", interaction, aiDmReply = false }: {
  commentPreview?: string; data: WizardData; mode: EditorPreviewMode; onModeChange: (mode: EditorPreviewMode) => void; username?: string | null; avatar?: string | null;
  ...
  const bubble = (message: string, buttons: string[] = [], outbound = false) => <div className={outbound ? styles.outbound : styles.message}><p dir="auto">{personalizeUsername(message,"username")}</p>{buttons.map((label, index) => <span key={index} dir="auto" className={styles.button}>{label}</span>)}</div>;
  return <section className={styles.preview} aria-label={tr("Instagram live preview")}>
    ...
    <p className={styles.caption}>{tr(mode === "dm" ? "This is a preview of your automated conversation." : mode === "comments" ? "This is how your reply appears under the triggering comment." : "This is the post people comment on to trigger your automation.")}</p>
  </section>;
}

...
export default function Ap3kAiConsole({ slug, initial, plan, initialPlaygroundMessages = [] }: { slug: string; initial: Profile; plan: string; initialPlaygroundMessages?: PlaygroundMessage[] }) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1480px] py-5 sm:py-8">
      ...
      {!unlocked ? <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-violet-400/25 bg-violet-500/[0.08] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-slate-950 dark:text-white">AP3K AI is included with Pro and Business</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400"><UiText>{"Upgrade to enable AI comments, AI DM replies, knowledge, and the playground."}</UiText></p></div><Link href={`/dashboard/${slug}/billing`} className="rounded-xl bg-violet-600 px-5 py-3 text-center text-sm font-black text-white">View plans</Link></div> : null}
...
function Playground({ unlocked, ready, initialMessages }: { unlocked: boolean; ready: boolean; initialMessages: PlaygroundMessage[] }) {
  ...
  return <div className="mx-auto flex min-h-[540px] max-w-3xl flex-col">
    ...
    <div className="mt-5 min-h-0 flex-1 space-y-3 overflow-y-auto rounded-2xl bg-slate-50/70 p-3 dark:bg-white/[0.02] sm:p-4">
      {!messages.length ? <div className="grid min-h-[280px] place-items-center text-center"><div><Sparkles className="mx-auto h-7 w-7 text-violet-500" /><p className="mt-3 font-black"><UiText>{"Ask a customer-style question"}</UiText></p><p className="mt-1 text-sm text-slate-500">Your conversation is saved. Tests count toward the monthly AI limit.</p></div></div> : null}

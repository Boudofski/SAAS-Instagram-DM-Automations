  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#0d0f19] shadow-2xl shadow-violet-950/30">
      <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-violet-300"><UiText>{"Interactive example"}</UiText></p>
          <p className="mt-1 text-sm font-bold text-white"><UiText>{"Comment → match → DM"}</UiText></p>
        </div>
        <button onClick={step ? () => { clearTimers(); setStep(0); } : run} className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-black text-violet-700 transition hover:-translate-y-0.5">
          ...
          <UiText>{step ? "Reset" : "Run example"}</UiText>
        </button>
      </div>
      <div className="grid min-h-[330px] gap-5 p-5 sm:p-7">
        <div className={`flex items-start gap-3 transition-all duration-300 ${step >= 1 ? "opacity-100" : "opacity-35"}`}>
          ...
          <div className="rounded-2xl rounded-tl-md bg-white/8 px-4 py-3 text-sm text-white"><UiMessage source="Please send me the {keyword}." values={{ keyword: <bdi>{keyword.toLowerCase()}</bdi> }} /></div>
        </div>
        <div className={`mx-auto flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-black transition-all duration-300 ${step >= 2 ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-white/10 text-white/30"}`}>
          <Check className="h-3.5 w-3.5" /><UiMessage source="Keyword {keyword} matched" values={{ keyword: <bdi>{keyword}</bdi> }} /></div>
        <div className={`ms-auto flex max-w-[90%] items-start gap-3 transition-all duration-300 ${step >= 3 ? "translate-y-0 opacity-100" : "translate-y-2 opacity-25"}`}>
          <div className="rounded-2xl rounded-tr-md bg-gradient-to-br from-violet-600 to-fuchsia-600 px-4 py-3 text-sm text-white shadow-lg"><UiText>{" Here’s what you asked for. "}</UiText><span className="mt-3 flex items-center justify-center gap-2 rounded-xl bg-white/16 px-4 py-2 font-black"><Send className="h-3.5 w-3.5" /><UiText>{" Open link"}</UiText></span>
          </div>
        </div>
      </div>
    </div>

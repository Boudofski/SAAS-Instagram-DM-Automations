...
export function DeleteUserButton({ userId, email, accountCount }: { userId: string; email: string; accountCount: number }) {
  ...
  return <AlertDialog open={open} onOpenChange={(value) => { if (!busy) { setOpen(value); setConfirmation(""); setReason(""); setError(""); } }}>
    <AlertDialogTrigger asChild><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-red-400/25 bg-red-500/10 px-4 text-sm font-bold text-red-300 hover:bg-red-500/20"><Trash2 className="h-4 w-4" />Delete AP3K account</button></AlertDialogTrigger>
    <AlertDialogContent className="max-h-[90dvh] w-[calc(100%-2rem)] overflow-y-auto border-red-400/20 bg-[#0c111d] text-white">
      <AlertDialogHeader><AlertDialogTitle>Permanently delete this AP3K account?</AlertDialogTitle><AlertDialogDescription className="space-y-3 text-slate-400">
      ...
      <label className="space-y-2 text-sm">Reason<textarea value={reason} onChange={e => setReason(e.target.value)} disabled={busy} maxLength={500} rows={2} className="mt-2 w-full rounded-xl border border-white/15 bg-white/5 p-3 text-base" /></label>
      <label className="space-y-2 text-sm">Type <strong className="font-mono">DELETE</strong><input value={confirmation} onChange={e => setConfirmation(e.target.value)} disabled={busy} autoComplete="off" spellCheck={false} className="mt-2 w-full rounded-xl border border-red-400/30 bg-white/5 p-3 text-base" /></label>

...
function Gallery() {
  ...
  return <div style={{ padding: 16, background: "#202431", minHeight: "100vh", color: "white", fontFamily: "system-ui" }}>
    <header style={{ display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center", marginBottom: 16 }}>
      ...
      <label>Workspace <select aria-label="Preview workspace" value={page} onChange={e => setPage(e.target.value)} style={{ color: "black" }}><option value="overview">Overview</option><option value="editor">Editor</option></select></label>
    </header>
...
async function start() {
  const content = params.get("page") === "editor" ? <EditorialEditor initial={BLOG_POSTS[0]} version={0} existing={true}/> : await Overview();
  createRoot(document.getElementById("root")!).render(params.has("frame") ? <div className="admin-shell dark min-h-screen bg-[#0b0e16] text-slate-100"><AdminV2Nav environment="UI fixture"/><main className="admin-main lg:pl-[248px]"><div className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">{content}</div></main></div> : <Gallery/>);
}
void start();

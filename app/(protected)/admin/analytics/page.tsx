        ...
        <div className="flex rounded-lg border border-white/10 p-1">
          {[7, 30].map((n) => (
            <Link
              ...
              className={`rounded-md px-4 py-2 text-sm ${days === n ? "bg-violet-500/15 text-violet-200" : "text-slate-400"}`}
            >
              {n} days
            </Link>
          ))}
  ...
  return (
    <section className="admin-panel">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span
          className={`rounded-full px-2.5 py-1 text-xs ${connected ? "bg-emerald-500/10 text-emerald-300" : "bg-amber-500/10 text-amber-200"}`}
        >
            ...
            <thead>
              <tr>
                {columns.map((c) => (
                  <th
                    key={c}
                    className="whitespace-nowrap border-b border-white/10 p-3 text-xs font-medium text-slate-400"
                  >
                    {c}
                  </th>
                ))}

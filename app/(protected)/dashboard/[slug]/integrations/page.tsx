...
const ERROR_COPY: Record<string, string> = {
  ...
  page_resolution_failed: "Meta authorization succeeded, but AP3k could not resolve the Instagram account. Reconnect and approve the requested Instagram permissions.",
  ig_business_not_linked: "Meta could not find an eligible Instagram Business or Creator account for this login.",
  page_token_missing: "AP3k could not validate access for the selected Instagram account.",
  no_eligible_facebook_pages: "No eligible Instagram professional account was found for this Meta login.",
  webhook_subscription_failed: "The account connected, but AP3k could not subscribe it to comment/message webhooks.",
  ...
  insufficient_developer_role: "Meta rejected the connection because this account does not have enough role access for this app while it is in development mode.",
  missing_code: "Instagram did not return an authorization code.",
  oauth_failed: "Instagram connection could not be completed.",
};

const DIRECT_ERROR_STEPS: Record<string, string[]> = {
  insufficient_developer_role: [
    "Add the Instagram account owner as a tester, developer, or administrator in Meta Developers.",
    "Accept the tester invitation for the Instagram account.",
    "Reconnect after the role is accepted.",
  ],
  ...
  webhook_subscription_failed: [
    ...
    "If it still fails, check the admin diagnostics for the safe Meta error.",
  ],
};

const LEGACY_ERROR_STEPS: Record<string, string[]> = {
  insufficient_developer_role: [
...
async function Page({ params, searchParams }: PageProps) {
  ...
  const recentPageContent =
    appReviewMode && !directInstagramLogin && instagram
  ...
  const errorMessage = error
    ...
    : null;
  const errorSteps = directInstagramLogin ? DIRECT_ERROR_STEPS : LEGACY_ERROR_STEPS;
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-1 py-4 text-slate-950 dark:text-slate-50 sm:px-2 lg:py-8">
      {error && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-relaxed text-red-800 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200">
          ...
          {(!appReviewMode || error === "no_eligible_facebook_pages") && errorSteps[error] && (
            <ul className="mt-3 list-disc space-y-1 pl-5">
              {errorSteps[error].map((step) => (
                <li key={step}>{step}</li>
              ))}
            </ul>
          )}
        ...
        <div className="ap3k-panel p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            {directInstagramLogin
              ? "Connect a Business or Creator Instagram account directly. AP3k subscribes comment and message webhooks automatically after authorization."
              : "Connect the Business or Creator Instagram account that owns the posts AP3k should monitor. This legacy setup may use the linked Facebook Page behind the scenes."}
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 text-sm leading-relaxed text-slate-600 shadow-sm dark:border-white/[0.12] dark:bg-white/[0.04] dark:text-slate-300">
        <p className="font-black text-slate-950 dark:text-white">Official Instagram workflow</p>
        <p className="mt-2">
          {directInstagramLogin
            ? "AP3k opens Instagram's official authorization screen, stores the returned Instagram access token, subscribes the account to comments and messages, and uses official Instagram APIs for replies."
      ...
      {appReviewMode && instagram && directInstagramLogin && (
        <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm dark:border-emerald-500/30 dark:bg-emerald-500/10">
          ...
          <Link
            ...
            className="ap3k-gradient-button mt-5 inline-flex min-h-11 items-center justify-center rounded-xl px-4 text-center text-sm font-black text-white"
          >
      ...
      {appReviewMode && instagram && !directInstagramLogin && (
        <section className="grid gap-5 lg:grid-cols-2">
          <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm dark:border-emerald-500/30 dark:bg-emerald-500/10">
            ...
            <h2 className="mt-2 text-2xl font-black text-slate-950 dark:text-white">
              {instagram.instagramUsername ? `@${instagram.instagramUsername}` : instagram.pageName ?? "Instagram connected"}
            </h2>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/[0.12] dark:bg-white/[0.04]">
            ...
            <h2 className="mt-2 text-2xl font-black text-slate-950 dark:text-white">
              {recentPageContent?.data?.pageName ?? instagram.pageName ?? "Selected account"}
            </h2>
            <p className="mt-4 text-sm leading-relaxed text-slate-600 dark:text-slate-300">
              AP3k verifies that the selected account can access recent content before continuing to comment automation.
            </p>
            ...
            {!recentPageContent?.error && recentPageContent?.data?.posts.length === 0 && (
              <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">
                Meta returned no recent posts for this account.
              </div>
            )}
            <Link
              href={`${dashboardPath(params.slug)}/automation/new`}

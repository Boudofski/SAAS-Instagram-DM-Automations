import ConversationPreview from "@/components/website/conversation-preview";
import GrowthFeatures from "@/components/website/growth-features";
...
export default async function LandingPage() {
  ...
  return (
    <LocalizedCopy><div className="ap3k-home min-h-screen overflow-x-clip bg-[#f7f7fb] text-slate-950 transition-colors dark:bg-[#080911] dark:text-white">
      <HomeScrollProgress />
      ...
      <main>
        <section className="ap3k-hero relative isolate px-4 pb-14 pt-10 sm:px-8 sm:pb-20 sm:pt-16 lg:pt-20">
          <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1.08fr_1fr] lg:gap-16">
            <FadeIn>
              <div className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white/80 px-3 py-2 text-xs font-semibold text-violet-700 dark:border-violet-400/20 dark:bg-violet-400/10 dark:text-violet-200"><Sparkles className="h-3.5 w-3.5" /> Instagram Comment &amp; DM Automation</div>
              <h1 className="ap3k-home-title mt-6 text-[2.6rem] font-extrabold leading-[1.08] tracking-[-0.055em] sm:text-6xl lg:text-[4.25rem]">Turn Instagram Comments <span className="text-violet-600 dark:text-violet-400">Into Customers.</span></h1>
              <p className="mt-6 max-w-xl text-base leading-7 text-slate-600 dark:text-slate-300 sm:text-lg sm:leading-8">Automatically reply to comments, send the promised link by DM, and track every lead—without complicated flows or code.</p>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row"><Link href="/sign-up" className="ap3k-gradient-button rounded-full px-7 py-3.5 text-sm">GET STARTED <ArrowRight className="h-4 w-4" /></Link><a href="#how-it-works" className="ap3k-outline-button rounded-full px-6 py-3.5 text-sm">See how it works</a></div>
              <p className="mt-4 text-xs leading-6 text-slate-500 dark:text-slate-400">500 automated actions every month. No credit card required.</p>
              <div aria-label="AP3K platform credentials" className="mt-6 flex flex-wrap gap-4 border-t border-slate-200/70 pt-5 dark:border-white/10"><span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><BadgeCheck className="h-4 w-4 text-violet-600 dark:text-violet-400" />Official Instagram API</span><span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><Sparkles className="h-4 w-4 text-violet-600 dark:text-violet-400" />Comment automation</span></div>
            </FadeIn>
            <FadeIn delay={0.08}><ConversationPreview /></FadeIn>
          </div>
        </section>
        <GrowthFeatures />
